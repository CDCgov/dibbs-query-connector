...
The `Patient Record` page contains your patient’s medical record. Depending on the data returned by your query, the page can include the following sections:
...
* Conditions
* Diagnostic Reports
* Medications (medication requests and medication statements)
* Immunizations
...
### What data does a query retrieve?

Query Connector retrieves patient data in the form of FHIR resources – the standard format healthcare systems use to exchange health data. Depending on how a query is built, results can include the following resource types:

* **Patient** – demographic information used to identify the patient, like name, date of birth, and contact information
* **Observation** – lab test results, as well as social determinants of health if they’re included in the query
* **DiagnosticReport** – lab reports and their associated findings
* **Condition** – diagnoses and other health concerns
* **Encounter** – healthcare visits related to the conditions included in the query
