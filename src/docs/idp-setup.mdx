...
1. Follow the [deployment instructions](./deployment.mdx) to set up Keycloak as your IdP for the Query Connector.
...
![Keycloak navigation panel](../../public/idp-config/keycloak_1.svg)

4. Select the `Roles` tab and create the following three roles (case sensitive). For a detailed explanation for each role, refer to our [user guide](./user-guide.mdx). You can add a description if you wish to make the roles understandable to other members of your organization.

   1. `super-admin` (notice the hyphen)
   2. `admin`
   3. `standard`

   ![](../../public/idp-config/keycloak_2.svg)

   After creation, you should have the following three roles available for assignment: super admin, admin, and standard.

   ![](../../public/idp-config/keycloak_3.svg)
...
5. Navigate to the Users tab in the left side nav: you should see a list of all the users within your organization that have access to Keycloak. Based on your organization’s structure, assign users to the role they should have within the Query Connector by selecting a user, navigating to the `Role mapping` tab, clicking `Assign role`, and filling out the associated details.
   ...
   ![](../../public/idp-config/keycloak_4.svg)

   ![](../../public/idp-config/keycloak_5.svg)

   ![](../../public/idp-config/keycloak_6.svg)
...
![](../../public/idp-config/menu_standard.svg)
...
- Admin users should have access to all the pages standard users do, and also have access to the query and code libraries and the user management page.
  ...
  ![](../../public/idp-config/menu_admin.svg)
...
- Super admin users should have access to all the pages admin users do, and should also have access to the FHIR server configuration page and the audit log.
  ...
  ![](../../public/idp-config/menu_superadmin.svg)
...
1.  Follow the [deployment instructions](./deployment.mdx) to set up Entra as your IdP for the Query Connector.
2.  Navigate to the [Entra admin console](https://entra.microsoft.com/) for your organization. There are two places you’ll need to configure access. Make sure that the account that you’re using is an owner in the Enterprise application (this will allow you to assign roles).
    ...
    ![](../../public/idp-config/azure_1.svg)

    And the app registration (which will allow you to create roles).![](../../public/idp-config/azure_2.svg)

    ![](../../public/idp-config/azure_3.svg)

3.  Navigate to the `App registrations` page under applications and click on the `App roles` tab. Create the following three roles by clicking `Create app role` at the top of the screen with the following values:![](../../public/idp-config/azure_4.svg)
    1.  Super Admin
...
4.  Navigate to `Applications > Enterprise applications` and select `Manage > Users and Groups`. Start the role assignment for each user by checking the box next to their name and then clicking `Edit Assignment`
    ...
    ![](../../public/idp-config/azure_5.svg)

5.  Assign roles by selecting the `None selected` link on the pane that appears and then selecting the correct role based on your organization’s structure. Save the selection by hitting `Assign` after the selection. ![](../../public/idp-config/azure_6.svg)  
    Azure should give you a confirmation after the assignment succeeds.

    ![](../../public/idp-config/azure_7.svg)
