...
- Interactive API documentation is available [here](https://queryconnector.dev/apiDocs).
...
## Queried FHIR Resources

Query results are returned as a FHIR Bundle containing the patient's records. Depending on the contents of the query being run, the bundle can include the following resource types:
...
- Check the [interactive API documentation](https://queryconnector.dev/apiDocs) for detailed endpoint information
