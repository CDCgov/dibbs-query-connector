...
The diagram below shows how these pieces fit together. Query Connector and its PostgreSQL database run in your own infrastructure—any cloud provider or on-premises—while the identity provider and FHIR servers are typically existing services you connect to.

![Query Connector deployment architecture: the Query Connector app and a managed PostgreSQL database run in the jurisdiction's own infrastructure (any cloud or on-premises), connecting to the jurisdiction's existing identity provider for authentication and to external FHIR servers owned by healthcare providers and HIEs](../../public/query-connector-reference-architecture.png)
...
4. Begin by initiating the bash setup script, which is called [dibbs-query-connector-wizard.sh](https://github.com/CDCgov/dibbs-vm/blob/main/dibbs-query-connector/dibbs-query-connector-wizard.sh.home).
...
13. Next, you’ll be prompted to enter secrets and API keys:
    ...
    - Next, enter your ERSD API key. [See here](./development.mdx#obtaining-api-and-license-keys) for instructions for obtaining these keys – there’s no default key, because each user should use their own.
...
5. **Database seeding**
   For production Docker deployments, the database needs to ingest valueset information from external valueset APIs (e.g., VSAC, UMLS) in order for the Query Connector to build useful queries. In order to do so, navigate to `/queryBuilding` as an admin user. There, you should see a screen prompting you to build your first query, which will kick off the valueset ingestion process. This process will take several minutes, so stay on the page until the process completes. Contact us for help if you run into issues!
...
For more information on the ERSD and UMLS API keys, see [this API key documentation](./development.mdx#obtaining-api-and-license-keys).
...
If you have feedback or questions, feel free to contact our team at dibbs@cdc.gov.
