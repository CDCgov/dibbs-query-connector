...
- Spin up a virtual machine (e.g. an [EC2 instance](https://aws.amazon.com/ec2/) or [Azure virtual machine](https://azure.microsoft.com/en-us/products/virtual-machines)) which has Docker installed, pull the image, and run it
...
Here we provide a basic example of pulling the Docker image onto an EC2 instance, but we are happy to provide guidance on the other options as well. 
...
- Using a container orchestration platform like ECS or a PaaS like Azure App Service will provide benefits like autoscaling and easier updates
...
7. Next, you’ll see a mention of the node environment, NODE_ENV {production} –  By default we assume that you’re deploying to a production environment. If needed, update this name to describe the environment you’re deploying to (test, development, etc.) .

8. Next, you’ll be presented with  a question about which database you’d like to connect to:  Do you have a pre-existing database available to connect to Query Connector? You can choose either yes or no – the default response is yes. Our intention is for our users to have their own managed database for any production instance of the VM. If you have a managed database, choose yes.
...
For more information on the ERSD and UMLS API keys, see [here](development.md#obtaining-api-and-license-keys).
...
3. **Disaster recovery plan**:
   ...
   - Document procedures for restoring the application from backups
